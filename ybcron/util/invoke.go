...
const (
	EXPIRE_RES_INFO time.Duration = 30 * time.Second
)
...
func get_info(uri string, app_id string, datakey string, datastu interface{}, notuserproxy bool, timeout time.Duration) (err error) {
	uri = Config.Servers[app_id] + uri
	appkey := Config.AppKeys[app_id]
	headers := make(map[string]string)
	headers["Host"] = Config.ServerHost[app_id]
