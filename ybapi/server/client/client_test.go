...
func TestLotterys_Record(t *testing.T) {
	router := gin.New()
	router.Use(func(c *gin.Context) {
		c.Set("user_id", int64(52041))
	})
	router.GET("/test", Lotterys_Record)
	// RUN
	w := performGetRequest(router, "/test?id=29")
	// TEST
	assert.Equal(t, w.Code, 200)
...
func TestLotterysSelfRecord(t *testing.T) {
	router := gin.New()
	router.Use(func(c *gin.Context) {
		c.Set("user_id", int64(52041))
	})
	router.GET("/test", Lotterys_SelfRecord)
	// RUN
	w := performGetRequest(router, "/test?status=2")
	// TEST
	assert.Equal(t, w.Code, 200)
...
func TestLotterys_Pay(t *testing.T) {
	router := gin.New()
	router.Use(func(c *gin.Context) {
		c.Set("user_id", int64(52041))
	})
	router.POST("/test", Lotterys_Pay)
	// RUN
	w := performPostRequest(router, "/test?id=1", `{"amount":1.25,"memo":"p1WPqhfSV9Ba","zjpassword":"148367352b16e9a0114b8f0bd97473bd60eb7321","coin_type":0,"lotterys_id":81}`)
	// TEST
	assert.Equal(t, w.Code, 200)
...
func TestLotterys_Confirm(t *testing.T) {
	router := gin.New()
	router.Use(func(c *gin.Context) {
		c.Set("user_id", int64(52041))
	})
	router.POST("/test", Lotterys_Confirm)
	// RUN
	w := performPostRequest(router, "/test?id=1", `{"address_id":150,"lotterys_record_id":161}`)
	// TEST
	assert.Equal(t, w.Code, 200)
...
func TestOfPayCard(t *testing.T) {
	...
	v, err := util.GetOfpay().CardWidthdraw(615615615135, "1232904", 1)
