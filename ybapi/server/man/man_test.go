...
func TestBusiness_Status(t *testing.T) {
	router := gin.New()
	router.Use(func(c *gin.Context) {
		c.Set("user_id", int64(51887))
	})
	router.POST("/test", Business_Status)
	// RUN
	w := performPostRequest(router, "/test", `{"user_id":1}`)
	// TEST
	assert.Equal(t, w.Code, 200)
...
func TestBusiness_AmountUpdate(t *testing.T) {
	router := gin.New()
	router.Use(func(c *gin.Context) {
		c.Set("user_id", int64(51887))
	})
	router.POST("/test", Business_AmountUpdate)
	// RUN
	w := performPostRequest(router, "/test", `[{"user_id":-1, "amount":0, "payamount":0}]`)
	// TEST
	assert.Equal(t, w.Code, 200)
...
func TestLotterys_Upsert(t *testing.T) {
	router := gin.New()
	router.Use(func(c *gin.Context) {
		c.Set("user_id", int64(51887))
	})
	router.POST("/test", Lotterys_Upsert)
	// RUN
	w := performPostRequest(router, "/test", `{"p_id":1,"coin":3,"price":200,"num":200,"stock":200,"amount":21000,"reward_ybt":55555,"start_time":1571911165,"end_time":1572083965}`)
	// TEST
	assert.Equal(t, w.Code, 200)
...
func TestLotterysList(t *testing.T) {
	router := gin.New()
	router.Use(func(c *gin.Context) {
		c.Set("user_id", int64(51903))
	})
	router.GET("/test", Lotterys_List)
	// RUN
...
func TestLotterysInfo(t *testing.T) {
	router := gin.New()
	router.Use(func(c *gin.Context) {
		c.Set("user_id", int64(51903))
	})
	router.GET("/test", Lotterys_Info)
	// RUN
	w := performGetRequest(router, "/test?id=1")
	// TEST
	assert.Equal(t, w.Code, 200)
...
func TestLotterys_RecordHash(t *testing.T) {
	router := gin.New()
	router.Use(func(c *gin.Context) {
		c.Set("user_id", int64(51903))
	})
	router.POST("/test", Lotterys_RecordHash)
	// RUN
	w := performPostRequest(router, "/test?id=1", `{"order_id":85,"rebat":0,"tx_hash":"5c52ae0276a40412b02ab303c81873f2af928f4769029e3ed4c33feee1c8aeb1"}`)
	// TEST
	assert.Equal(t, w.Code, 200)
