...
func AESEncrypt(token string) string {
	src := []byte(token)
...
func AESDecrypt(token string) (ok_token string, err error) {
    var ok_token_bt []byte
	ok_token_bt, err = base64.StdEncoding.DecodeString(token)
	if err != nil {
		glog.Error("base64.DecodeString(", token, ") failed! err:", err)
		return
	}
