...
type yunexAccount struct {
	UserId  int64  `json:"user_id,string"`
	Address string `json:"string" binding:"required"`
}
...
func IsYunexAddress(address, coin string) (user_id int64, err error) {
	...
	uri := fmt.Sprintf("/api/pub/address/check/?plat=yunbay&coin=%v&address=%v", strings.ToUpper(coin), address)
	var ret addrYunex
...
// 获取yunex平台帐户余额信息
func GetYunExBalance() (dataStatus map[string]string, err error) {
