...
# 生成token
```
curl 'http://account.yunbay.com:92/man/YBAccount/token?user_id=380'
```

### 发送一个消息到nsq服务中
```
curl -H"Content-Type: application/json" 'http://127.0.0.1:4151/pub?topic=mqurl' \
-d '{"delay":5000000,"method":"POST","appkey":"ybapi","uri":"/man/business/amount/update/am","headers":null,"data":[{"user_id":55221,"amount":1,"rebat":"0.99"}],"timeout":0,"maxtrys":0}'
```


### 登录请求
```
curl -H"Content-Type: application/json" -H"X-Yf-AppId: account" -H"X-Yf-rid: 1" -H"X-Yf-Platform: test" -H"X-Yf-Version: 2.8.1" -H"X-Yf-Sign: 62361670a0b60c852fcc1e69189c233e" http://127.0.0.1:92/v1/YBAccount/login -d '{"user_id":51887,"amount":0.5,"order_ids":[1604374], "subject":"新订单有"}'
```
...
### 关闭交易订单
```
curl -v -X POST     -H'X-YF-SIGN: 62361670a0b60c852fcc1e69189c233e' -H'Host: 172.17.6.140' -H'X-YF-AppId: Account' -H'X-YF-rid: 1' -H'X-YF-Platform: man' -H'X-YF-Version: 1.0.1' -H'Content-Type: application/json' 'http://172.17.6.140:2006/man/trade/close' -d '{}'
```