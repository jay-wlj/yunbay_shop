...
// 发送中将消息
func (t *Notify) NotifyRetMsg(id int64, status int, content string, uids []int64) (err error) {
...
// 发送hash更新消息
func (t *Notify) NotifyHash(to, lotterys_record_id int64, num_hash string) (err error) {
	ext := make(map[string]interface{})
...
// 发送状态消息
func (t *Notify) NotifyStatus(lotterys_id int64, sold, status int) (err error) {
	ext := make(map[string]interface{})
	ext["id"] = lotterys_id
	ext["sold"] = sold
	ext["status"] = status
...
// 发送卡密订单状态消息
func (t *Notify) NotifyOfCardStatus(order_id, user_id int64, cards []common.OfCard, status int) (err error) {
	ext := make(map[string]interface{})
	ext["id"] = order_id
	ext["cards"] = cards
	ext["status"] = status
	m := msg{Type: 0, Action: "order_ret", To: []int64{user_id}, Data: ext, Ack: false} // 无需回复
